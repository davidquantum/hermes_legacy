...
MatrixSolverType matrix_solver = SOLVER_MUMPS;  // Possibilities: SOLVER_AMESOS, SOLVER_AZTECOO, SOLVER_MUMPS,
...
#define COARSE
  ...
  //basemesh.refine_all_elements(1); // when only p-adapt is used and const voltage
  basemesh.refine_towards_boundary(BDY_TOP, REF_INIT);
  basemesh.refine_towards_boundary(BDY_SIDE, REF_INIT);  // only when nonconstant voltage used
  basemesh.refine_towards_boundary(BDY_BOT, (REF_INIT - 1) + 8); // when only p-adapt is used
  basemesh.refine_towards_boundary(BDY_BOT, REF_INIT - 1);
#endif
